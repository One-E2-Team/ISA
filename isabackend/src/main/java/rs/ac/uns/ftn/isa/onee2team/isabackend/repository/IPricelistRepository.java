...
public interface IPricelistRepository extends JpaRepository<Pricelist, Long> {
	...
	@Query(value = "select p.price from pricelists p where p.pharmacy_id = ?1 and p.medicine_id = ?2 and p.start_date < ?3 and p.end_date > ?3", nativeQuery = true)
	Double getPricelistForMedicineInPharmacyForDate(Long pharmacyId, Long medicineId, Date date);
}
